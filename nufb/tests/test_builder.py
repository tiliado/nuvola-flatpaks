...
class TestBuilder:
    ...
    def test_set_up_from_scratch(self, tmp_path: Path, manifest: Manifest):
        ...
        builder = Builder(Task(build_root, tmp_path, manifest))
    ...
    def test_set_up_dirty(self, tmp_path: Path, manifest: Manifest):
        ...
        build_root = tmp_path / 'blah-blah' / 'root'
        builder = Builder(Task(build_root, tmp_path, manifest))
        builder.task.manifest_json.parent.mkdir(parents=True)
        builder.task.manifest_json.write_text('abc')
        sub_dir = builder.task.build_dir / 'sub-dir'
    ...
    def test_clean_up_nothing(self, tmp_path: Path, manifest: Manifest):
        ...
        build_root = tmp_path / 'blah-blah' / 'root'
        builder = Builder(Task(build_root, tmp_path, manifest))
        assert not builder.task.build_dir.is_dir()
        builder.clean_up()
    ...
    def test_clean_up_dirty(self, tmp_path: Path, manifest: Manifest):
        ...
        builder.task.manifest_json.parent.mkdir(parents=True)
        builder.task.manifest_json.write_text('abc')
        builder.task.build_dir.is_dir()
        sub_dir = builder.task.build_dir / 'sub-dir'
        sub_dir.mkdir(parents=True)
        builder.clean_up()
        assert not builder.task.build_dir.exists()
        assert not builder.task.manifest_json.exists()
        assert not sub_dir.exists()
    ...
    def test_copy_resources_success(self, tmp_path: Path):
        ...
        builder = Builder(Task(build_root, resources_dir, manifest))
        builder.copy_resources()
        for path in files:
            assert (builder.task.build_dir / path).read_text() \
